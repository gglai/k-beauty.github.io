...
function checkColor(color) {
    ...
    if (color != "default") {
        $(".flowList.for-cur").css({ "border": "2px solid #EBC6C3"});
        $(".flowList.for-cur,.flowListBox .for-cur em").css({ "background-color": "#EBC6C3"});
        $(".flowListBox .for-cur em").css({ "border": "0px none #000" , "color": "	#000000"}); 
    } else {
    ...
    /*让当前选中步骤变为深蓝色*/
	 ...
	 obj.css({ "border": "2px solid #A35353", "background-color": "#A35353" });
	 obj.children("em").css({ "background-color": "#A35353" , "color": "#FFFFFF"}); 
}
//确定步骤的宽度占比
...
function loadFlow(count){
  ...
  for(var i=1;i<=count;i++){
    ...
    if(i==1){
      ...
      flowVar += "	<em style='position:absolute;left:43%'>"+i+"</em><br/><strong style='position:absolute;left:43%'>第"+i+"步</strong>\n";
      flowVar += "</div>\n";
    }else{
